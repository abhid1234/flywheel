...
export function buildAtlas(episodes, clusters, opts = {}) {
  try {
    ...
    return {
      ...
      totals: { episodes: source.length, failures: labels.fail, passes: labels.pass, unknown: labels.unknown, replayableWitnesses, projects: projects.size, sessions: sessions.size },
      tierBreakdown,
      failureRate: ratio(labels.fail, labeled),
      byErrorClass: countRows(classes, labels.fail, "errorClass"),
      byTool: countRows(tools, labels.fail, "tool", "failures"),
      topClusters,
...
<div class="stats"><div class="stat"><strong>${finite(totals.episodes)}</strong><span>episodes</span></div><div class="stat"><strong>${percent(data.failureRate)}</strong><span>failure rate among labeled episodes</span></div><div class="stat"><strong>${finite(totals.replayableWitnesses)}</strong><span>replayable witnesses</span></div><div class="stat"><strong>${proposable}</strong><span>proposable clusters</span></div></div>
<section><h2>Failures by error class</h2><ul class="bars">${errorBars}</ul></section>
