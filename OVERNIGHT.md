...
- [ ] G2  `flywheel gold` command: read factory-episodes.jsonl + GitHub merge-status, emit gold labels,
        merge into episode outcomes. Report gold count vs the n>=60 floor honestly.
- [ ] G3  A/A calibration harness (`flywheel calibrate`): run the deterministic witnesses N times,
        confirm ~0 variance; scaffold the agent-trial A/A (bounded) for when gold arrives. KC-2.
- [ ] G4  Deploy flywheel to the mini; a launchd/cron that harvests+labels+reports every few hours
...
- [ ] G7  Honest findings writeup (blog draft) — the KC-6 result, the 4-fix integration story, the atlas.
